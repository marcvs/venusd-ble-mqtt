...
venusd-ble-mqtt --list-commands # list supported BLE opcodes (no hardware/config)
venusd-ble-mqtt --address AA:BB:CC:DD:EE:FF --once --publish-raw --log-level DEBUG  # one cycle, for bring-up
```
...
- **`ble.py`** (`VenusBLE`): async context manager over `bleak`. Resolves device by `address` or by scanning for an advertised-name prefix. The device answers a command by streaming notification chunks; `_on_notify` reassembles them into frames using the length byte and a header-resync fallback, pushing complete frames onto an asyncio queue. `request()` drains stale frames, writes the command, and waits (with timeout) for one response frame.
...
- Config files (`*.ini`) are gitignored except `venusd-ble-mqtt.example.ini` — update the example when adding config keys.
